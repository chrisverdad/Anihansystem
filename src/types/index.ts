...
export interface SourceWasteSubmission {
  ...
  /** Populated vendor from API; includes optional business_name when joined from users. */
  vendor?: User
}
...
/** Vendor info embedded on catalog products (from products.vendor_id → users) */
...
export interface Product {
  ...
  unit: 'kg' | 'bags' | 'bottles' | 'pieces' | 'jars' | 'boxes'
  is_available: boolean
  /** When false, hidden from unauthenticated landing (GET /products?public=1). Default true. */
  is_public?: boolean
  created_at: string
  updated_at: string
  /** Set when the product is tied to a vendor (e.g. auto-created from source waste) */
  vendor_id?: string
  vendor?: ProductVendorInfo
  /** When set, admin removed the product from the public catalog (soft delete); row kept in DB. */
  deleted_at?: string | null
}

/** Courier / fulfillment row from `deliveries` (vendor-managed); shown to buyers on order details. */
...
export interface Delivery {
  id: string
  order_id: string
  /** Fulfilling vendor (from product); used to scope vendor delivery UI */
  vendor_id?: string
  delivery_person?: string
  delivery_vehicle?: string
  pickup_time?: string
  ...
  notes?: string
  /** Internal notes; returned only to admin (stripped for vendor API responses) */
  admin_notes?: string
  created_at: string
  updated_at: string
  order?: Order
