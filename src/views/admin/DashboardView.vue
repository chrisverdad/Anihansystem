          ...
          <div class="card-body">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <div class="ml-3">
                  <p class="text-sm font-medium text-gray-900">Reports & analytics</p>
                  <p class="text-sm text-gray-500">Charts, exports, and metrics</p>
                </div>
              </router-link>
...
const createWasteChart = () => {
  ...
  const statusCounts = {
    scheduled: wasteData.filter(s => s.status === 'pending' || s.status === 'approved' || s.status === 'rejected').length,
    collected: wasteData.filter(s => s.status === 'collected').length,
  ...
  new ChartJS(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Scheduled', 'Collected', 'Processed'],
      datasets: [{
        data: [statusCounts.scheduled, statusCounts.collected, statusCounts.processed],
        backgroundColor: [
          '#6366F1', // Indigo scheduled
          '#F59E0B', // Amber collected
          '#10B981'  // Emerald processed
        ],
        borderWidth: 2,
        borderColor: '#ffffff'
      }]
