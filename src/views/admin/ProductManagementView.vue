          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="product in paginatedProducts" :key="product.id" class="hover:bg-gray-50">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      ...
                      <span v-if="product.deleted_at" class="inline-block mt-1 badge-danger text-xs">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-gray-900">{{ vendorColumnLabel(product) }}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      v-if="!product.deleted_at"
                      type="button"
                      @click="removeFromCatalog(product.id)"
        ...
        <div class="mt-3">
          ...
          <form @submit.prevent="saveProduct" class="space-y-6">
            ...
            <div class="form-group">
              <label class="flex items-center">
                <input
                  v-model="productForm.is_public"
                  type="checkbox"
                  class="rounded border-gray-300 text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50"
                />
                <span class="ml-2 text-sm text-gray-700">Show on public home page (logged-out visitors)</span>
              </label>
            </div>
...
const vendorColumnLabel = (product: Product) => {
  if (!product.vendor) return '—'
  const b = (product.vendor.business_name || '').trim()
  return b || (product.vendor.full_name || '').trim() || '—'
}
...
const filteredProducts = computed(() => {
  ...
  if (searchQuery.value.trim()) {
    ...
    filtered = filtered.filter(product => {
      const idMatch = String(product.id).toLowerCase().includes(q)
      const nameMatch = product.name.toLowerCase().includes(q)
      const descMatch = product.description.toLowerCase().includes(q)
      const v = product.vendor
      const vendorFull = (v?.full_name ?? '').toLowerCase()
      const vendorBiz = (v?.business_name ?? '').toLowerCase()
      const vendorEmail = (v?.email ?? '').toLowerCase()
      const vendorMatch =
        vendorFull.includes(q) || vendorBiz.includes(q) || vendorEmail.includes(q)
...
const loadProducts = async () => {
  try {
    await productsStore.loadProducts({ includeRemoved: true })
...
const removeFromCatalog = async (productId: string) => {
  if (
    !confirm(
      'Remove this product from the public catalog? The record stays in the database for orders and reports.'
    )
  ) {
    return
  }
  try {
    await productsStore.deleteProduct(String(productId), { includeRemoved: true })
...
const restoreProduct = async (productId: string) => {
  try {
    await productsStore.restoreProduct(String(productId))
