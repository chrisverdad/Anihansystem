<template>
  <nav
    class="bg-white/85 backdrop-blur-md border-b border-gray-200/80 shadow-sm fixed w-full top-0 z-50 supports-[backdrop-filter]:bg-white/75"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        ...
        <div class="hidden md:flex items-center gap-0.5">
          <router-link
            ...
            :key="item.name"
            :to="item.href"
            class="text-gray-600 hover:text-violet-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 whitespace-nowrap"
            :class="{
              'text-violet-700 bg-violet-50 ring-1 ring-violet-600/10': $route.path === item.href
            }"
          >
            {{ item.name }}
          </router-link>
        ...
        <div class="flex items-center space-x-3">
          ...
          <button
            type="button"
            class="p-2 text-gray-500 hover:text-violet-600 hover:bg-violet-50 rounded-lg relative transition-colors"
            aria-label="Notifications"
          >
            <BellIcon class="h-5 w-5" />
            <span
              class="absolute top-1 right-1 h-2 w-2 bg-violet-500 rounded-full ring-2 ring-white"
              aria-hidden="true"
            />
          </button>
          ...
          <div class="relative" ref="profileDropdown">
            ...
            <div
              ...
              class="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-lg py-1.5 z-50 border border-gray-200/80 ring-1 ring-gray-900/5"
            >
              <router-link
                to="/profile"
                class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-violet-50 hover:text-violet-800 rounded-lg mx-1"
              ...
              <button
                type="button"
                @click="handleLogout"
                class="block w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg mx-1"
              >
                Sign Out
              </button>
            </div>
      ...
      <!-- Mobile Navigation -->
      <div v-if="showMobileMenu" class="md:hidden border-t border-gray-100 py-3 pb-4">
        <div class="space-y-0.5">
          <router-link
            v-for="item in navigationItems"
            :key="item.name"
            :to="item.href"
            class="block px-3 py-2.5 text-sm font-medium text-gray-700 hover:text-violet-700 hover:bg-violet-50 rounded-lg"
            :class="{
              'text-violet-700 bg-violet-50 ring-1 ring-violet-600/10': $route.path === item.href
            }"
...
const navigationItems = computed(() => {
  ...
  if (authStore.isAdmin) {
    baseItems.push(
      ...
      { name: 'Reports & analytics', href: '/admin/reports' }
    )
    ...
    baseItems.push(
      ...
      { name: 'Deliveries', href: '/vendor/deliveries' },
